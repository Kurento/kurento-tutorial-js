...
* *ws_url*: the WebSocket Kurento MediaServer endpoint. By default it connect to
  a Kurento MediaServer instance listening on the port 8888 on the same machine
  where it's being hosted the demo.
* *ice_servers*: the TURN and STUN servers to use, formatted as a JSON string
  ...
  ```
  http://example.com/index.html?ice_servers=[{"urls":"stun:stun1.example.net"}]
  ```
